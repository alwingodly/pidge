...
import OnlinePaymentsToggle from "@/components/admin/OnlinePaymentsToggle"
...
export default async function PaymentsSettingsPage({
  ...
  const tenant = await prisma.tenant.findUnique({
    where:  { id: tenantId },
    select: { stripeAccountId: true, stripeOnboarded: true, onlinePaymentsEnabled: true },
  })
    ...
    <div className="space-y-4">
      ...
      <div className="overflow-hidden rounded-xl border border-border bg-white shadow-sm">
        <div className="flex items-start gap-3 border-b border-[#F3EAE0] px-5 py-4">
          ...
          <div>
            <p className="text-sm font-bold text-foreground">Online payments</p>
            <p className="mt-0.5 text-xs text-muted-foreground">
              When off, patients see no payment step — they pay in person (cash / offline).
            </p>
          </div>
        </div>
        <div className="px-5 py-4">
          <OnlinePaymentsToggle initialEnabled={tenant?.onlinePaymentsEnabled ?? false} />
        </div>
      </div>

      {/* Stripe connect — only relevant when online payments is on */}
      <div className="overflow-hidden rounded-xl border border-border bg-white shadow-sm">
        <div className="flex items-start gap-3 border-b border-[#F3EAE0] px-5 py-4">
          <div className="flex size-8 shrink-0 items-center justify-center rounded-lg bg-secondary text-primary">
            <CreditCard className="size-4" />
          ...
          <div>
            <p className="text-sm font-bold text-foreground">Stripe account</p>
            <p className="mt-0.5 text-xs text-muted-foreground">
